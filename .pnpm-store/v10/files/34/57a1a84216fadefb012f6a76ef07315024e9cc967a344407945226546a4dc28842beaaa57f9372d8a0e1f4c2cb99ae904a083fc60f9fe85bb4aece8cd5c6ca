...
const ASTRO_PATH_TOKEN_PARAM = "x_astro_path_token";
...
const getIsrPath = (pathToken) => `/_isr?${ASTRO_PATH_PARAM}=$0&${ASTRO_PATH_TOKEN_PARAM}=${pathToken}`;
      ...
      "astro:config:setup": async ({ command, config, updateConfig, injectScript, logger }) => {
        ...
        updateConfig({
          build: {
            format: "directory",
            redirects: false,
            ...config.output === "static" ? { server: new URL("./.vercel/output/server/", config.root) } : {}
          },
          integrations: [
            {
          ...
          if (_buildOutput === "server" && isr) {
            ...
            if (isrConfig.exclude?.length) {
              const expandedExclusions = isrConfig.exclude.flatMap((exclusion) => {
                if (exclusion instanceof RegExp) {
                  return routes.filter((route) => exclusion.test(route.pattern)).map((route) => route.pattern);
                }
                return [exclusion];
              });
            ...
            for (const route of routes) {
              ...
              if (!excludeRouteFromIsr) {
                ...
                const isInternal = src.startsWith("^\\/_image") || src.startsWith("^\\/_server-islands");
...
class VercelBuilder {
  ...
  async buildMiddlewareFolder(entry, functionName, middlewareSecret, isrForwarding) {
    const functionFolder = new URL(`./functions/${functionName}.func/`, this.outDir);
